...
class JobFinder(object):
    ...
    def setupGUI(self):
        ...
        states = ttk.Combobox(searchFrame, state="readonly", width=5, values=["CA", "PA", "OH"]).grid(row=0, column=2,
                                                                                                      padx=5)
        tk.Label(searchFrame, text="Job Category", bg=color).grid(row=0, column=3)
        ttk.Combobox(searchFrame, state="readonly", width=15,
                     values=["Data Scientist", "Software Developing Engineer", "Consultant"]).grid(row=0,
                                                                                                   column=4, padx=5)
        ...
        self.lbCourseList = tk.Listbox(courseListFrame, height=25, width=30, yscrollcommand=courseListScrollbar.set,
                                       relief=SUNKEN)
        ...
        self.labCourseDescription = tk.Text(courseDescriptionFrame, wrap=WORD, height=17, width=50,
                                            yscrollcommand=courseDescriptionScrollbar.set, relief=SUNKEN)
